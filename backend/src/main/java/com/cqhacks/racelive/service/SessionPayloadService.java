...
import com.cqhacks.racelive.demo.LapTimesReader;
import com.cqhacks.racelive.demo.Race;
import com.cqhacks.racelive.demo.RaceResultsReader;
...
public class SessionPayloadService {
    ...
    public SessionPayloadService(DataPathsProperties paths) {
        this.paths = paths;
    }

    public Optional<SessionPayload> buildForRound(int round) {
        Path lapPath = paths.getLapTimesCsv();
        Path rrPath = paths.getRaceResultsCsv();
        LapTimesReader ltReader = new LapTimesReader(lapPath.toString());
        ArrayList<Race> races = new ArrayList<>(ltReader.readCSV());
        if (races.isEmpty()) {
            return Optional.empty();
        }
        RaceResultsReader rrReader = new RaceResultsReader(rrPath.toString());
    ...
    private static List<SessionStandingsRow> buildStandings(Race race) {
        ...
        for (SortRow row : rows) {
            ...
            String tc = (d.teamColor != null && !d.teamColor.isBlank()) ? d.teamColor : "#888888";
            out.add(
