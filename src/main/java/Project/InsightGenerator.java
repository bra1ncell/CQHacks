...
public class InsightGenerator {
    ...
    public ArrayList<String> generateAll() {
        ...
        // Remove any null entries produced when data was insufficient
    ...
    String fastestLap() {
        ...
        for (Driver driver : drivers) {
            for (Lap lap : driver.getLaps()) {
                if (!hasValue(lap.lapTime)) continue;
                if (best == null || toMillis(lap.lapTime) < toMillis(best.lapTime)) {
                    best = lap; bestDriver = driver;
                }
            }
        }
        if (best == null) return null;
        return "Fastest lap: " + formatTime(best.lapTime)
                + " by " + driverName(bestDriver)
                + " (Lap " + fmt(best.lapNumber) + ")";
    }
        ...
        for (Driver driver : drivers) {
            for (Lap lap : driver.getLaps()) {
                ...
                if (worst == null || toMillis(lap.lapTime) > toMillis(worst.lapTime)) {
                    worst = lap; worstDriver = driver;
                }
            }
        }
        if (worst == null) return null;
        return "Slowest lap: " + formatTime(worst.lapTime)
                + " by " + driverName(worstDriver)
                + " (Lap " + fmt(worst.lapNumber) + ")";
    }
    ...
    String mostConsistentDriver() {
        ...
        return "Most consistent driver: " + bestDriver
                + " (fastest–slowest lap gap: " + msToDisplay(bestRange) + ")";
    }
    ...
    String biggestPaceImprovement() {
        ...
        for (Driver driver : drivers) {
            Lap firstValid = null, lastValid = null;
            for (Lap lap : driver.getLaps()) {
                if (!hasValue(lap.lapTime)) continue;
                if (firstValid == null) firstValid = lap;
                lastValid = lap;
            }
            if (firstValid == null || lastValid == null || firstValid == lastValid) continue;
        ...
        if (bestDriver == null || bestDelta <= 0) return null;
        return "Biggest pace improvement lap 1 → last lap: "
                + bestDriver + " improved by " + msToDisplay(bestDelta);
    }

    // ── Sectors ───────────────────────────────────────────────────────────────

    List<String> fastestSectors() {
        Lap    bestS1 = null, bestS2 = null, bestS3 = null;
        Driver dS1    = null, dS2    = null, dS3    = null;

        for (Driver driver : drivers) {
            for (Lap lap : driver.getLaps()) {
                if (hasValue(lap.sector1Time) &&
                        (bestS1 == null || toMillis(lap.sector1Time) < toMillis(bestS1.sector1Time))) {
                    bestS1 = lap; dS1 = driver;
                }
                if (hasValue(lap.sector2Time) &&
                        (bestS2 == null || toMillis(lap.sector2Time) < toMillis(bestS2.sector2Time))) {
                    bestS2 = lap; dS2 = driver;
                }
                if (hasValue(lap.sector3Time) &&
                        (bestS3 == null || toMillis(lap.sector3Time) < toMillis(bestS3.sector3Time))) {
                    bestS3 = lap; dS3 = driver;
                }
            }
        }

        List<String> results = new ArrayList<>();
        if (bestS1 != null) results.add("Fastest S1: " + formatTime(bestS1.sector1Time) + " by " + driverName(dS1));
        if (bestS2 != null) results.add("Fastest S2: " + formatTime(bestS2.sector2Time) + " by " + driverName(dS2));
        if (bestS3 != null) results.add("Fastest S3: " + formatTime(bestS3.sector3Time) + " by " + driverName(dS3));
        return results;
    }
    ...
    String bestTheoreticalLap() {
        ...
        for (Driver driver : drivers) {
            long minS1 = Long.MAX_VALUE, minS2 = Long.MAX_VALUE, minS3 = Long.MAX_VALUE;
            for (Lap lap : driver.getLaps()) {
                if (hasValue(lap.sector1Time)) minS1 = Math.min(minS1, toMillis(lap.sector1Time));
                if (hasValue(lap.sector2Time)) minS2 = Math.min(minS2, toMillis(lap.sector2Time));
                if (hasValue(lap.sector3Time)) minS3 = Math.min(minS3, toMillis(lap.sector3Time));
            }
            if (minS1 == Long.MAX_VALUE || minS2 == Long.MAX_VALUE || minS3 == Long.MAX_VALUE) continue;
        ...
        if (bestDriver == null) return null;
        return "Best theoretical lap (sum of personal best sectors): "
                + msToDisplay(bestTotal) + " by " + bestDriver;
    }
    ...
    List<String> highestTopSpeedPerTeam() {
        ...
        teamBest.entrySet().stream()
                ...
                .forEach(e -> results.add(
                        "Top speed [" + e.getKey() + "]: "
                                + String.format("%.1f km/h", e.getValue())));
        return results;
    }
    ...
    String longestStint() {
        ...
        if (bestDriver == null) return null;
        return "Longest stint: " + bestLength + " laps on "
                + fmt(bestCompound) + " by " + bestDriver;
    }
    ...
    String mostPitStops() {
        ...
        for (Driver driver : drivers) {
            int pits = 0;
            for (Lap lap : driver.getLaps()) {
                if (hasValue(lap.pitInTime)) pits++;
            }
            if (pits > mostPits) {
                mostPits   = pits;
        ...
        if (mostDriver == null) return null;
    ...
    String fastestCompound() {
        ...
        for (Driver driver : drivers) {
            for (Lap lap : driver.getLaps()) {
                if (!hasValue(lap.lapTime) || !hasValue(lap.compound)) continue;
        ...
        if (bestCompound == null) return null;
        return "Fastest average lap by compound: "
                + bestCompound + " (" + msToDisplay(bestAvg) + " avg)";
    }
    ...
    String freshVsWornLapDelta() {
        ...
        for (Driver driver : drivers) {
            long freshTotal = 0, wornTotal = 0;
            int  freshCount = 0, wornCount = 0;

            for (Lap lap : driver.getLaps()) {
                if (!hasValue(lap.lapTime)) continue;
                long ms = toMillis(lap.lapTime);
                if (lap.freshTyre) { freshTotal += ms; freshCount++; }
                else               { wornTotal  += ms; wornCount++;  }
            }
            if (freshCount == 0 || wornCount == 0) continue;
        ...
        if (biggestDeltaDriver == null || biggestDelta <= 0) return null;
        return "Biggest fresh vs worn tyre delta: " + biggestDeltaDriver
                + " (" + msToDisplay(biggestDelta) + " slower on worn tyres on average)";
    }
    ...
    String mostPositionsGained() {
        ...
        for (Driver driver : drivers) {
            Double startPos = null, endPos = null;
            for (Lap lap : driver.getLaps()) {
                if (lap.position == null) continue;
                if (startPos == null) startPos = lap.position;
                endPos = lap.position;
            }
        ...
        if (bestDriver == null || bestGain <= 0) return null;
    ...
    List<String> mostLapsLed() {
        ...
        for (Driver driver : drivers) {
            int led = 0;
            for (Lap lap : driver.getLaps()) {
                if (lap.position != null && lap.position == 1.0) led++;
            }
            if (led > 0) lapsInLead.put(driverName(driver), led);
        }
        List<String> results = new ArrayList<>();
    ...
    String mostDeletedLaps() {
        ...
        for (Driver driver : drivers) {
            ...
            for (Lap lap : driver.getLaps()) {
                if (lap.deleted) {
                    deleted++;
                    if (hasValue(lap.deletedReason)) reason = lap.deletedReason;
                }
            }
        ...
        if (mostDriver == null || mostDeleted == 0) return null;
        return "Most deleted laps: " + mostDriver + " (" + mostDeleted + " laps"
    ...
    String accuracyRate() {
        ...
        if (bestDriver == null) return null;
        return "Highest lap accuracy rate: " + bestDriver
                + " (" + String.format("%.0f%%", bestRate) + " accurate laps)";
    }

    // ── Time parsing helpers ──────────────────────────────────────────────────

    private long toMillis(String time) {
        if (time == null || time.isBlank()) return Long.MAX_VALUE;
        try {
            String[] parts    = time.trim().split(" ");   // ["0","days","HH:MM:SS.ffffff"]
            String[] timePart = parts[2].split("[:.]");   // ["HH","MM","SS","ffffff"]
    ...
    private String formatTime(String time) {
        ...
        if (ms == Long.MAX_VALUE) return time;
